...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "`$env:JAVA_HOME='C:\Program Files\Java\jdk-21'; cd '$PSScriptRoot'; ./mvnw spring-boot:run -pl user-service" -WindowStyle Normal
Write-Host "-> User Service iniciado (Puerto 8080)" -ForegroundColor Green

# Iniciar Payment Service
Start-Process powershell -ArgumentList "-NoExit", "-Command", "`$env:JAVA_HOME='C:\Program Files\Java\jdk-21'; cd '$PSScriptRoot'; ./mvnw spring-boot:run -pl payment-service" -WindowStyle Normal
...
Start-Process powershell -ArgumentList "-NoExit", "-Command", "`$env:JAVA_HOME='C:\Program Files\Java\jdk-21'; cd '$PSScriptRoot'; ./mvnw spring-boot:run -pl notification-service" -WindowStyle Normal
Write-Host "-> Notification Service iniciado (Puerto 8083)" -ForegroundColor Green

# Iniciar Gateway Service
Start-Process powershell -ArgumentList "-NoExit", "-Command", "`$env:JAVA_HOME='C:\Program Files\Java\jdk-21'; cd '$PSScriptRoot'; ./mvnw spring-boot:run -pl gateway-service" -WindowStyle Normal
